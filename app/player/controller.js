...
module.exports = {
  ...
  checkout: async (req, res) => {
    try {
      ...
      if (!res_nominal) return res.status(404).json({ message: `Voucher tidak ditemukan` });
      const res_bank = await Bank.findOne({ _id: bank });
      if (!res_bank) return res.status(404).json({ message: `Voucher tidak ditemukan` });
      const res_payment = await Payment.findOne({ _id: payment });
      if (!res_payment) return res.status(404).json({ message: `Voucher tidak ditemukan` });
    ...
    try {
      ...
      category.forEach((element) => {
        count.forEach((result) => {
          if (result._id.toString() === element._id.toString()) {
    ...
    try {
      ...
      if (req.file) {
        ...
        src.on("end", async () => {
          ...
          let currImage = `${config.rootPath}/public/uploads/${player.avatar}`;
